...
class APFObstacleAvoidance(Node):
    ...
    def control_loop(self):
        ...
        if self.obstacle_detected:
            ...
            if total_force > 0 and total_force < 1100:
                # print("hi_prior_last")
            ...
            elif total_force > 1100:
                desired_angle = atan2(force_y, force_x)
                angular_error = desired_angle - self.robot_orientation
                while angular_error > 3.14: angular_error -= 6.28
                while angular_error < -3.14: angular_error += 6.28
                if angular_error > 0:
                    n_angle = angular_error + 1.57
                elif angular_error < 0:
                    n_angle = angular_error -1.57

                start_time = time.time()
                while time.time() - start_time < 1.2:
                    cmd_vel.angular.z = 0.4 * n_angle
                ...
                while time.time() - move_start_time <1.2:
                    cmd_vel.linear.x = 0.3
                    cmd_vel.angular.z = 0.0
                    print("moving a bit")
                    self.cmd_vel_pub.publish(cmd_vel)
